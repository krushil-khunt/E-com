...
<head>
    ...
    <title>PhoneHub | Update Device</title>
    <!-- Fonts -->
...
<body class="h-full font-sans antialiased bg-slate-50/50">
    ...
    <div class="min-h-screen flex flex-col pb-12">
            ...
            <div class="max-w-4xl mx-auto flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-[#14213D] flex items-center justify-center shadow-md">
                        <i class="fas fa-edit text-sm text-[#FCA311]"></i>
                    </div>
                    <div>
                        <h1 class="text-base font-bold font-outfit text-slate-900 tracking-tight">
                            Update Device
                        </h1>
                        <p class="text-[9px] text-[#FCA311] font-bold tracking-wide uppercase">Edit Catalog Entry</p>
                    </div>
                </div>
        ...
        <main class="max-w-4xl w-full mx-auto px-6 mt-8 flex-grow">
            <!-- Alert success if updated in session -->
            @if (session('success'))
            ...
            <div class="glass-card rounded-3xl p-6 sm:p-8">
                <form action="{{ url('editdata/'.$product->id) }}" method="post" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="name" value="{{ $product->name }}" required class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200 shadow-sm">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="brand" value="{{ $product->brand }}" required class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200 shadow-sm">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" value="{{ $product->price }}" required class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200 shadow-sm">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" name="ram" value="{{ $product->ram }}" required class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200 shadow-sm">
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" name="storage" value="{{ $product->storage }}" required class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200 shadow-sm">
                                    </div>
                                </div>
                            </div>
           ...
           class="form-control">
                        ...
                        <div class="space-y-5 flex flex-col justify-between">
                            ...
                            <div>
                                ...
                                <textarea name="description" required rows="4" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-2.5 px-4 text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200 resize-none shadow-sm">{{ $product->description }}</textarea>
                            </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <div class="border-2 border-dashed border-slate-200 group-hover:border-indigo-500 bg-slate-50/50 rounded-xl p-4 text-center transition duration-200 flex flex-col items-center justify-center">
                                            ...
                                            <span class="text-xs font-semibold text-slate-600 group-hover:text-slate-800">Upload New</span>
                                    ...
                                    <div id="imagePreviewContainer" class="w-full h-28 rounded-xl bg-slate-50 border border-slate-250 flex items-center justify-center text-slate-400 p-2 overflow-hidden shadow-inner">
                                        @if($product->image && (str_starts_with($product->image, 'http') || file_exists(public_path('uploads/' . $product->image))))
                                            <img id="imagePreview" src="{{ str_starts_with($product->image, 'http') ? $product->image : asset('uploads/' . $product->image) }}" alt="Preview" class="object-contain max-h-full max-w-full rounded-lg">
                                            <div id="previewFallback" class="hidden text-center">
                                        ...
                                        @else
                                            <div id="previewFallback" class="text-center">
                                                <i class="fas fa-image text-xl mb-1"></i>
                                                <p class="text-[9px]">No image uploaded</p>
                                            </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                        <a href="{{ url('display') }}" class="inline-flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-slate-500 hover:text-slate-700 text-xs font-semibold py-3 px-6 rounded-xl border border-slate-200 transition duration-200 shadow-sm">
                            Cancel
                        </a>
